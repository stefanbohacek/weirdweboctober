...
ready(async () => {
  const statusesUrl = `https://stefanbohacek.online/api/v1/accounts/113495724129260623/statuses?limit=1`;
  const statusesResp = await fetch(statusesUrl);
  const statuses = await statusesResp.json();
  const mostRecentPost = statuses[0];
  const mostRecentPostContainer = document.getElementById("most-recent-post");

  mostRecentPostContainer.innerHTML = /* html */ `
    <blockquote class="mastodon-embed"
      data-embed-url="${mostRecentPost.url}/embed"
      ...
      <a href="${mostRecentPost.url}"
        ...
        <div style="color: #787588; margin-top: 16px;">Post by @emoji__polls@stefanbohacek.online</div>
...
`;
  ...
  script.src = "https://stefanbohacek.online/embed.js";
  script.async = true;
  script.setAttribute("data-allowed-prefixes", "https://stefanbohacek.online/");
